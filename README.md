...
│   ├── quiz.js             測驗引擎與後端回傳
│   └── data-w1.js … data-w5.js   由 tools/extract-questions.js 自動產生，供錯題複習頁使用
...
## 下一週要加內容時（W5 已完成，下一個是 W6，STEP 2 知識整理第二週）

W1～W5 都已經開通、可以隨時回頭補進度，所以**不要**覆蓋既有的 `wN-index.html` 或
`dayN.html`——新週次一律用新檔名（W6 就用 `w6-index.html`、`day26.html`…`day30.html`、`w6-test.html`）。
`tools/extract-questions.js` 最下面也要照樣加一組 `buildWeek([...], 'W6_QUESTIONS', 'data-w6.js')`。
