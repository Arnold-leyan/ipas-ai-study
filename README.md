...
**W4（9/7–9/11）** — 學習指引 科目二 3.3 節，p.3-31～3-51（Day 5 另快速複習科目一 3.4、科目二 3.1）
...
| `day20.html` | 快速複習週：鑑別式AI／生成式AI 概念、No Code 回顧 | 科目一 p.3-48～3-49、科目二 p.3-1～3-16 摘要 | 10 |
| `w4-test.html` | W4 總測驗（Day 1～Day 5 整週） | p.3-31～3-51 | 20 |

共 34 題，其中 13 題是學習指引的官方原題，21 題依指引內容自編。
本週範圍在 114.04 版勘誤表中僅一處更正：p.3-32 第③項評估標準標題由「資源與基礎設施評估」
更正為「企業導入階段性實施策略」，已在 `day16.html` 採用更正後版本。
...
│   ├── quiz.js             測驗引擎與後端回傳
│   └── data-w1.js … data-w4.js   由 tools/extract-questions.js 自動產生，供錯題複習頁使用
...
## 下一週要加內容時（例如 W5，STEP 1 之後的知識整理週）

W1～W4 都已經開通、可以隨時回頭補進度，所以**不要**覆蓋既有的 `wN-index.html` 或
`dayN.html`——新週次一律用新檔名（W5 就用 `w5-index.html`、`day21.html`…`day25.html`、`w5-test.html`）。
